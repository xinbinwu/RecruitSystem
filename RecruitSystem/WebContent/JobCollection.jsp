...
<html lang="en">
	...
	<body>
		...
		<div class="inner main-container" id="option-keyword" default-value="职位 公司" value="">
          ...
          <a href="/interact/employee_favorite" class="content-tab current" data-id="technical_job">技术职位</a>
          <a href="/interact/employee_favorite/1" class="content-tab " data-id="other_job">其他职位</a>
        </h5>
			<div id="favorite-job" class="content-tab-box" data-id="technical_job" style="display: block;">
				<div class="deliver-box clearfix">
					<!-- 左侧 -->
					<div class="content-s deliver-box-l pull-left">
						<h5>收藏职位
     					<a href="javascript:;" class="location pull-right manageBtn">批量管理</a>
     				</h5>
						<div class="favorite-operation">
							<span class="tip pull-left">请选择</span>
							<a href="javascript:;" class="pull-right cancelBtn">取消收藏</a>
						</div>
					...
					<div class="deliver-box-r pull-right hidden" id="job-content">
						<h5>
						<a href="javascript:;" name="un-favorite" value="{jid}" class="pull-right">取消收藏</a>
					</h5>
						<div class="deliver-box-r-content">
		...
		<!-- 模态框（Modal） -->
		<div class="modal fade" id="cancelTipModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
					...
					success: function(res) {
						if(res.data.result == 1) {
							if(JSON.stringify(res.data.job) != '{}') {
								...
								$("#init-content a[name='un-favorite']").click(function() {
									...
									$.ajax({
										...
										success: function(res) {
											if(res.data.result == 1) {
												...
												leftObj.slideUp("slow");
												var job_content = $('#no-content').html();
												$('#init-content').html(job_content);
												leftObj.next().trigger("click");
											} else {
												//alert('取消收藏失败，请稍后再试!');
												$('#modal-tip-msg').html('取消收藏失败，请稍后再试!');
