...
<body>
	...
	<div class="inner main-container" id="option-keyword"
		...
		<h5 class="content-title">
			<a href="/interact/employee_deliver" class="content-tab current"
				data-id="technical_job">技术职位</a> <a
				href="/interact/employee_deliver/1" class="content-tab "
				data-id="other_job">其他职位</a>
		</h5>
		<div class="content-tab-box" data-id="technical_job"
			style="display: block;">
			<!-- 筛选 -->
			<div class="filter-box" value="" style="margin-bottom: 0;">
				<dl class="clearfix" id="time-t">
					<dt>投递时间：</dt>
					<dd>
						<a href="#" class="current" value="0">不限</a> <a href="#" value="1">今天</a>
						<a href="#" value="2">一周内</a> <a href="#" value="3">一个月内</a> <a
							href="#" value="4">三个月内</a> <a href="#" value="5">三个月以上</a>
					</dd>
				</dl>
			</div>
	...
	<script>
		...
		function tplReplace(html, data) {
			...
			var new_html = html.replace('{deliver_at}', data.deliver_at)
					.replace('{job_title}', data.job.job_title).replace(
							'{jid}', data.job.jid).replace('{salary_low}',
		...
		$('#time-t a').click(
				function() {
					...
					$(".deliver-filter-box li[value='0']").html(
							'待查看(' + status0 + ')');
					$(".deliver-filter-box li[value='3']").html(
							'有意向(' + status3 + ')');
					$(".deliver-filter-box li[value='1']").html(
							'不合适(' + status1 + ')');
